...
#SELECT sites.domain_name AS website, COUNT(leads.leads_id) AS number_of_leads, DATE(leads.registered_datetime) AS date_generated FROM leads LEFT JOIN sites ON leads.site_id = sites.site_id WHERE leads.registered_datetime BETWEEN '2011-01-01' AND '2011-02-15' GROUP BY sites.domain_name ORDER BY sites.domain_name;
...
#SELECT sites.domain_name AS website, COUNT(leads.leads_id) AS number_of_leads, DATE(leads.registered_datetime) AS date_generated FROM leads LEFT JOIN sites ON leads.site_id = sites.site_id WHERE leads.registered_datetime BETWEEN '2011-01-01' AND '2011-12-31' GROUP BY sites.site_id;
